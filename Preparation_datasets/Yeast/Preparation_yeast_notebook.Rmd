---
title: "Preparation yeast dataset for PTM project"
...
The yeast (Saccharomyces cerevisiae; strain ATCC 204508 / S288c) proteome was obtained from UniProtKB/Swiss-Prot database (reference proteome UP000002311; release 2022_02). Aggregation prone regions (APRs) were predicted computationally using TANGO at physiological conditions (pH at 7.5, temperature at 298 K, protein concentration at 1 mM and ionic strength at 0.15 M). 

```{r, include=FALSE}
tango_APRs <- read.delim("Data/Yeast/TANGO_output/PSX_tangoresidue.out", header=TRUE)
...
names90 <- read.delim("Data/Yeast/UniProt/yeast_90.fasta", header=FALSE)
...
```{r, include=FALSE}
#First save yeast proteome file
yeast_proteome <- aggregate (Residue ~ Protein, data=master_df, FUN="paste",collapse = "")
colnames(yeast_proteome) <- c("Protein","Sequence")
save(yeast_proteome, file = "Data/RData/Yeast/yeast_proteome.RData")
#Transmembrane domains , topology and signal peptides from deepTMHMM (deepTMHMM using biolib webserver)
TMRs_yeast <- read.table("Data/Yeast/TMHMM/TMRs_yeast.gff3", sep=";", quote="\"")
TMHMM <- subset(TMRs_yeast, V1 != "//")
...
TMHMM$Protein <- res
TMHMM <- subset(TMHMM, Protein %in% yeast_proteome$Protein)
...
rm(dt2,dt1)
gc()
save(master_df, file = "Data/RData/Yeast/all_tango_res_filtered.RData")
```
...
load("Data/RData/Yeast/all_tango_res_filtered.RData")
...
save(master_df3, file = "Data/RData/Yeast/all_tango_res_annotated.RData")
#Summarize TANGO information
load("Data/RData/Yeast/all_tango_res_annotated.RData")
...
save(master_df3, file = "Data/RData/Yeast/master_df.RData")
save(tango_APRs, file = "Data/RData/Yeast/tango_APRs_ALL.RData")
save(tango_APRs2, file = "Data/RData/Yeast/tango_APRs.RData")
```
...
```{r, include=FALSE}
load("Data/RData/Yeast/master_df.RData")
...
write.fasta(as.list(pure_tango$Residue), pure_tango$APRcount_tango, "Data/Yeast/TANGO_output/Pure_tango/pure_tango.fa", open = "w", nbchar = 60, as.string = FALSE)
pure_tango <- read.delim("Data/Yeast/TANGO_output/Pure_tango/PSX_globaltotal.out", header=FALSE)
...
```{r, include=FALSE}
uniprot.proteome <- read.delim("Data/Yeast/UniProt/yeast_proteome_info.tsv")
...
nlinked <- subset(PTMs_df, Type2 == "N-linked Glycosylation_N" & names == "YEAST", select = c(1,2,4,6))
...
bio_yeast = matrix(numeric(length(types)*9), nrow = length(types), ncol = 8)
colnames(bio_yeast) <- c("Distal region","FR3 N-ter","FR2 N-ter","FR1 N-ter","APR","FR1 C-ter","FR2 C-ter","FR3 C-ter")
rownames(bio_yeast) <- types
for (i in 1:length(types)){
  ...
  bio_yeast[i,] <- dat2[1:8] / dat2[9]
  print(dat2[1:8] / dat2[9])
  print(nrow(glyc))
}
save(bio_yeast, file = "Data/RData/Yeast/bio_yeast.RData")
