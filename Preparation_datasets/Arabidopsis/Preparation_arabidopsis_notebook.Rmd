---
title: "Preparation arabidopsis dataset for PTM project"
...
The Arabidopsis thaliana proteome was obtained from UniProtKB/Swiss-Prot database (reference proteome UP000006548; release 2022_02). Aggregation prone regions (APRs) were predicted computationally using TANGO at physiological conditions (pH at 7.5, temperature at 298 K, protein concentration at 1 mM and ionic strength at 0.15 M). 

```{r, include=FALSE}
tango_APRs <- read.delim("Data/Arabidopsis/TANGO_output/PSX_tangoresidue.out", header=TRUE)
...
names90 <- read.delim("Data/Arabidopsis/UniProt/arabidopsis_90.fasta", header=FALSE)
...
```{r, include=FALSE}
#First save arabidopsis proteome file
arabidopsis_proteome <- aggregate (Residue ~ Protein, data=master_df, FUN="paste",collapse = "")
colnames(arabidopsis_proteome) <- c("Protein","Sequence")
save(arabidopsis_proteome, file = "Data/RData/Arabidopsis/arabidopsis_proteome.RData")
#Create files for SignalP and TMHMM
write.fasta(as.list(arabidopsis_proteome$Sequence[1:5000]), arabidopsis_proteome$Protein[1:5000], "Data/Arabidopsis/UniProt/1-5000.fa", open = "w", nbchar = 60, as.string = FALSE)
write.fasta(as.list(arabidopsis_proteome$Sequence[5001:10000]), arabidopsis_proteome$Protein[5001:10000], "Data/Arabidopsis/UniProt/5001-10000.fa", open = "w", nbchar = 60, as.string = FALSE)
write.fasta(as.list(arabidopsis_proteome$Sequence[10001:nrow(arabidopsis_proteome)]), arabidopsis_proteome$Protein[10001:nrow(arabidopsis_proteome)], "Data/Arabidopsis/UniProt/10001-end.fa", open = "w", nbchar = 60, as.string = FALSE)
#Transmembrane domains , topology and signal peptides from deepTMHMM (deepTMHMM using biolib webserver)
trans1.5000 <- read.table("Data/Arabidopsis/TMHMM/TMRs_1-5000.gff3", sep=";", quote="\"")
trans5001.10000 <- read.table("Data/Arabidopsis/TMHMM/TMRs_5001-10000.gff3", sep=";", quote="\"")
transend <- read.table("Data/Arabidopsis/TMHMM/TMRs_10001-end.gff3", sep=";", quote="\"")
...
TMHMM <- subset(TMHMM, Protein %in% arabidopsis_proteome$Protein)
...
rm(dt2,dt1)
gc()
save(master_df, file = "Data/RData/Arabidopsis/all_tango_res_filtered.RData")
```
...
load("Data/RData/Arabidopsis/all_tango_res_filtered.RData")
...
save(master_df3, file = "Data/RData/Arabidopsis/all_tango_res_annotated.RData")
#Summarize TANGO information
load("Data/RData/Arabidopsis/all_tango_res_annotated.RData")
...
save(master_df3, file = "Data/RData/Arabidopsis/master_df.RData")
save(tango_APRs, file = "Data/RData/Arabidopsis/tango_APRs_ALL.RData")
save(tango_APRs2, file = "Data/RData/Arabidopsis/tango_APRs.RData")
```
...
```{r, include=FALSE}
load("Data/RData/Arabidopsis/master_df.RData")
...
write.fasta(as.list(pure_tango$Residue), pure_tango$APRcount_tango, "Data/Arabidopsis/TANGO_output/Pure_tango/pure_tango.fa", open = "w", nbchar = 60, as.string = FALSE)
pure_tango <- read.delim("Data/Arabidopsis/TANGO_output/Pure_tango/PSX_globaltotal.out", header=FALSE)
...
```{r, include=FALSE}
uniprot.proteome <- read.delim("Data/Arabidopsis/UniProt/arabidopsis_info.tsv")
...
nlinked <- subset(PTMs_df, Type2 == "N-linked Glycosylation_N" & names == "ARATH", select = c(1,2,4,6))
...
bio_arabidopsis = matrix(numeric(length(types)*9), nrow = length(types), ncol = 8)
colnames(bio_arabidopsis) <- c("Distal region","FR3 N-ter","FR2 N-ter","FR1 N-ter","APR","FR1 C-ter","FR2 C-ter","FR3 C-ter")
rownames(bio_arabidopsis) <- types
for (i in 1:length(types)){
  ...
  bio_arabidopsis[i,] <- dat2[1:8] / dat2[9]
  print(dat2[1:8] / dat2[9])
  print(nrow(glyc))
}
save(bio_arabidopsis, file = "Data/RData/Arabidopsis/bio_arabidopsis.RData")
