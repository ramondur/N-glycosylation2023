---
title: "Preparation drosophila dataset for PTM project"
...
The fruit fly (Drosophila melanogaster) proteome was obtained from UniProtKB/Swiss-Prot database (reference proteome UP000000803; release 2022_02). Aggregation prone regions (APRs) were predicted computationally using TANGO at physiological conditions (pH at 7.5, temperature at 298 K, protein concentration at 1 mM and ionic strength at 0.15 M). 

```{r, include=FALSE}
tango_APRs <- read.delim("Data/Drosophila/TANGO_output/PSX_tangoresidue.out", header=TRUE)
...
names90 <- read.delim("Data/Drosophila/UniProt/drosophila_90.fasta", header=FALSE)
...
```{r, include=FALSE}
#First save drosophila proteome file
drosophila_proteome <- aggregate (Residue ~ Protein, data=master_df, FUN="paste",collapse = "")
colnames(drosophila_proteome) <- c("Protein","Sequence")
save(drosophila_proteome, file = "Data/RData/Drosophila/drosophila_proteome.RData")
#Transmembrane domains , topology and signal peptides from deepTMHMM (deepTMHMM using biolib webserver)
TMRs_drosophila <- read.table("Data/Drosophila/TMHMM/TMRs_drosophila.gff3", sep=";", quote="\"")
TMHMM <- subset(TMRs_drosophila, V1 != "//")
...
TMHMM$Protein <- res
TMHMM <- subset(TMHMM, Protein %in% drosophila_proteome$Protein)
...
rm(dt2,dt1)
gc()
save(master_df, file = "Data/RData/Drosophila/all_tango_res_filtered.RData")
```
...
load("Data/RData/Drosophila/all_tango_res_filtered.RData")
...
save(master_df3, file = "Data/RData/Drosophila/all_tango_res_annotated.RData")
#Summarize TANGO information
load("Data/RData/Drosophila/all_tango_res_annotated.RData")
...
save(master_df3, file = "Data/RData/Drosophila/master_df.RData")
save(tango_APRs, file = "Data/RData/Drosophila/tango_APRs_ALL.RData")
save(tango_APRs2, file = "Data/RData/Drosophila/tango_APRs.RData")
```
...
```{r, include=FALSE}
load("Data/RData/Drosophila/master_df.RData")
...
write.fasta(as.list(pure_tango$Residue), pure_tango$APRcount_tango, "Data/Drosophila/TANGO_output/Pure_tango/pure_tango.fa", open = "w", nbchar = 60, as.string = FALSE)
pure_tango <- read.delim("Data/Drosophila/TANGO_output/Pure_tango/PSX_globaltotal.out", header=FALSE)
...
```{r, include=FALSE}
uniprot.proteome <- read.delim("Data/Drosophila/UniProt/drosophila_proteome_info.tsv")
...
nlinked <- subset(PTMs_df, Type2 == "N-linked Glycosylation_N" & names == "DROME", select = c(1,2,4,6))
...
bio_drosophila = matrix(numeric(length(types)*9), nrow = length(types), ncol = 8)
colnames(bio_drosophila) <- c("Distal region","FR3 N-ter","FR2 N-ter","FR1 N-ter","APR","FR1 C-ter","FR2 C-ter","FR3 C-ter")
rownames(bio_drosophila) <- types
for (i in 1:length(types)){
  ...
  bio_drosophila[i,] <- dat2[1:8] / dat2[9]
  print(dat2[1:8] / dat2[9])
  print(nrow(glyc))
}
save(bio_drosophila, file = "Data/RData/Drosophila/bio_drosophila.RData")
